...
async function loadAll(){
  const[a,b,c,d,e,f,g,h]=await Promise.all([
    ...
    fetch('/api/ai-for-science').then(r=>r.json()),
    fetch('/api/execution').then(r=>r.json()),
    fetch('/api/external-resources').then(r=>r.json()).catch(()=>[]),
    fetch('/api/dashboard').then(r=>r.json()).catch(()=>({})),
  ]);
  D.overview=a; D.dims=b; D.refs=d; D.afs=e; D.exec=f; D.extRes=g; D.dash=h;
  D.comps={}; c.forEach(x=>{D.comps[x.dimension]=x});
  try{
    D.insights=await fetch('/api/insights').then(r=>r.json());
  }catch(e){D.insights={}}
  // 평가 결과 및 근거 인덱스 (비동기 로드, 없으면 빈 객체)
  try{
    const[ev,ei]=await Promise.all([
      fetch('/api/evaluation/summary').then(r=>r.json()).catch(()=>({})),
      fetch('/api/evidence').then(r=>r.json()).catch(()=>({})),
    ]);
    D.evalSummary=ev; D.evidenceIndex=ei;
  }catch(e){D.evalSummary={}; D.evidenceIndex={}}
}
